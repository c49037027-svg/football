...
def predict_fixtures(model, fixtures: list[dict]) -> list[dict]:
    """對賽程算 1X2（含主場優勢）。fixtures=[{home, away, time?}]。

    回 [{home, away, time, p_home, p_draw, p_away}]；不在模型的球隊跳過。
    """
    ...
    for f in fixtures:
        ...
        if not h or not a or h not in model.attack or a not in model.attack:
            continue
        p = markets.outcome_1x2(model.score_matrix(h, a))   # neutral=False→主場優勢
    ...
    for code, (_, zh_name) in LEAGUES.items():
        ...
        if not preds:
            ...
            trs = "".join(
                ...
                f"<td class='tm'>{_h.escape(zh(p['away']))}</td></tr>"
                for p in preds)
            body = ("<table><thead><tr><th>主隊</th><th>主勝</th><th>和</th>"
                    "<th>客勝</th><th>客隊</th></tr></thead>"
