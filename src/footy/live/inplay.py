...
def final_score_matrix(model: DixonColesModel, home: str, away: str,
                       ...
                       rate_scale: float = 1.0, total_minutes: int = 90) -> np.ndarray:
    """整場最終比分的條件機率矩陣（給定目前比分與時間）。

    把剩餘進球矩陣平移 (home_goals, away_goals)，並擴大尺寸以容納。
    """
    rem = remaining_score_matrix(model, home, away, minute, rate_scale, total_minutes)
    n = rem.shape[0]
