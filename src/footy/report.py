...
# 世界盃是否結束（write_worldcup_site 依 wc_over 設定）：True → 導覽/首頁換季外版
_WC_OVER = False


def _navbar(active: str) -> str:
    """頂部導覽列。賽會期間以世界盃為主；賽會結束後改以 MLB/走地為主、世界盃收存檔。"""
    def cls(k):
        return " class='on'" if k == active else ""
    # /live 只存在於 Render 動態站；靜態站（GitHub Pages）需以 FOOTY_LIVE_URL
    # 環境變數（建置時）注入 Render 絕對網址，否則會 404。
    import os
    live_url = os.environ.get("FOOTY_LIVE_URL", "/live")
    # 生產：讀環境變數（各頁在 write_worldcup_site 設旗標前就可能先建好，如 mlb.html）；
...
def odds_quota_banner() -> str:
    """置頂用的額度橫幅（比 footer 徽章顯眼）。無紀錄回空字串。"""
    b = odds_quota_badge()
    if not b:
        return ""
    zero = "已用完" in b
    style = ("background:rgba(224,122,95,.10);border:1px solid rgba(224,122,95,.45)"
             if zero else
             "background:rgba(255,255,255,.03);border:1px solid var(--line)")
    return (f"<div class='small' style='{style};border-radius:9px;"
            f"padding:7px 12px;margin:0 0 12px'>{b}</div>")


def odds_quota_badge() -> str:
    """the-odds-api 額度徽章（剩餘/總量）；無紀錄回空字串。低於 15% 標紅。"""
    try:
        from .live.providers import read_quota
        q = read_quota()
    except Exception:  # noqa: BLE001
        q = None
    if not q or q.get("remaining") is None:
        return ""
    rem, total = q["remaining"], q.get("total")
    tot_s = f" / {total}" if total else ""
    at = (q.get("at") or "")[5:16].replace("T", " ")
    if rem <= 0:      # 歸零＝抓不到盤口的真正原因，要醒目
        return (f"<span style='color:#e07a5f;font-weight:700' title='更新於 {at} UTC'>"
                f"🎟️ 盤口額度已用完（0{tot_s}）——本頁盤口為模型自取線，"
                f"下月 1 日重置</span>")
    low = total and rem <= max(30, total * 0.15)
    color = "var(--warn)" if low else "var(--muted)"
    return (f"<span style='color:{color}' title='更新於 {at} UTC'>"
            f"🎟️ 盤口額度 {rem}{tot_s}{'（快用完）' if low else ''}</span>")


def _season_status(month: int) -> dict:
    """各運動當月是否在季（概略月份）。用於 hub 卡片狀態字。"""
    return {"mlb": 4 <= month <= 10, "nba": month >= 10 or month <= 6,
            "soccer": month >= 8 or month <= 5}


def render_leagues_placeholder(title: str = "足球五大聯賽") -> str:
    """五大聯賽空狀態頁（球季未開打時）。Part C 的聯賽產生器上線後由其取代。"""
    return f"""<!doctype html><html lang="zh-Hant"><head>
<meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">
...
def render_hub_html(title: str = "體育預測 · 首頁", month: int = 7) -> str:
    ...
    def card(href, ic, ti, st):
        return (f"<a href='{href}'><div class='card'><div class='ic'>{ic}</div>"
                f"<div class='ti'>{ti}</div><div class='st'>{st}</div></div></a>")
    cards = [
        card("mlb.html", "⚾", "MLB 今日預測",
             "球季進行中 · 每日預測+盤口" if sea["mlb"] else "球季空檔（4-10 月）"),
        card(live_url, "🔴", "走地即時", "MLB 進行中比賽的即時勝率/公平賠率"),
        card("nba.html", "🏀", "NBA 今日預測",
...
def _mlb_mrow(market: str, pick: str, prob: float, sig=None) -> str:
    ...
    fair = 1.0 / max(prob, 0.005)
    return (f"<div class='mrow'><span class='mk'>{market}</span>"
            f"<span class='pk'>{pick}{_bs_badge(sig.get('verdict'))}</span>"
            f"<span class='pb' title='模型公平賠率 {fair:.2f}'>{prob:.0%}"
            f"<span class='dim' style='font-weight:400'>·{fair:.2f}</span></span>"
...
def write_worldcup_site(result, model, matches, outdir, history=None,
                        ...
                        wc_over=False, leagues_html=None):
    """產生多頁網站：index.html + 每場可分析的 match_<num>.html。

    wc_over=True（賽會結束）：世界盃頁改寫 wc.html、index.html 改入口 hub、
    加 leagues.html（足球聯賽，空窗為佔位）；導覽列同步換季外版。
    ...
    global _WC_OVER
    _WC_OVER = wc_over          # 供 _navbar 換版（單進程建站，安全）
    outdir = Path(outdir)
    outdir.mkdir(parents=True, exist_ok=True)
    injury_counts = injury_counts or {}
    wc_back = "wc.html" if wc_over else "index.html"
    ...
    for m in matches:
        ...
        if t1 in model.attack and t2 in model.attack:
            ...
            html_doc = render_analysis_html(a, page_title, back_href=wc_back,
                                            interactive=interactive)
            (outdir / f"match_{m.num}.html").write_text(html_doc, encoding="utf-8")
            linked.add(m.num)

    # 世界盃奪冠頁：賽會期間＝index.html；結束＝wc.html（存檔），index 改 hub
    wc_page = "wc.html" if wc_over else "index.html"
    write_worldcup_html(result, model, matches, outdir / wc_page, title, linked,
                        track_text=track_text)
    if wc_over:
        import datetime as _d2
        (outdir / "index.html").write_text(
            render_hub_html(month=_d2.date.today().month), encoding="utf-8")
        (outdir / "leagues.html").write_text(
            leagues_html or render_leagues_placeholder(), encoding="utf-8")
